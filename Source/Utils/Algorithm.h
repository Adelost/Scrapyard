...
template<class T>
static void Algorithm::swap(T& left, T& right)
{
	return std::swap(left, right);
}

template<class T>
T Algorithm::maximum(const T& value)
{
	return value;
}

template<class T, typename... Args>
T Algorithm::maximum(const T& value, Args... args)
{
	return std::max(maximum(args...), value);
}

template<class T, class U>
static bool Algorithm::any(const T& value, const U& arg)
{
	return value == arg;
}
template<class T, class U, class... Args>
static bool Algorithm::any(const T& value, const U& arg, Args... args)
{
